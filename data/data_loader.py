...
class ENTDataset(Dataset):
    def __init__(self, data, tokenizer, segmentor=None, entity_dict=None, ent2type=None, wanted_type=None,
                 istrain=True):
        self.data = data
    ...
    def sequence_padding(self, inputs, length=None, value=0, seq_dims=1, pad_mode='constant', mode='post'):
        ...
        for x in inputs:
            ...
            x = np.pad(x, pad_width, pad_mode, constant_values=value)
            outputs.append(x)
        return np.array(outputs)

    def span_segment(self, text, start_mapping, end_mapping):
        span_list = []
        term_list = []
        cut_list = self.segmentor.cut(text.lower())
        start = 0
        for term in cut_list:
            ...
            if start in start_mapping and end in end_mapping:
                ...
                span = (start_l, end)
                if term in self.entity_dict.keys() and self.ent2type[term] in self.wanted_type and span[-1] < max_len:
                    span_list.append(span)
                    term_list.append(int(self.entity_dict[term]))
    ...
    def collate(self, examples):
        raw_text_list, batch_input_ids, batch_attention_mask, batch_labels, batch_segment_ids, batch_span_list, batch_term_list = [], [], [], [], [], [], []
        for item in examples:
            ...
            # 实体识别
            span_list, term_list = self.span_segment(raw_text, start_mapping, end_mapping)
            batch_span_list.append(span_list)
            batch_term_list.append(term_list)
        ...
        batch_spanlist = self.sequence_padding(batch_span_list)
        batch_spanlist = torch.LongTensor(batch_spanlist)
        return raw_text_list, batch_inputids, batch_attentionmask, batch_segmentids, batch_labels, batch_spanlist, batch_term_list
...
class REDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        text = text if len(text) <= self.config['max_len'] else text[:self.config['max_len']]
        sample = list(text)
